/**
 * VIEW — SpeedGameScreen (TS, token-based)
 * Étirer layout + auto-advance. The card is a MODULE-LEVEL component (DuelCard)
 * on purpose: defining it inside the screen would give it a new identity every
 * render, remounting it and killing the bar/number transitions.
 */

import { useSpeedGame, SPEED_MAX, type Choice } from "../../controllers/speedGame.controller";
...
const SPEED_COLOR = "#818cf8";
const WIN = "#4ade80";
const LOSE = "#f87171";

type Side = Exclude<Choice, "tie">;

function DuelCard({ side, c, revealed, locked, isWinner, isWrong, opponentSpeed, lang, onSelect }: {
  side: Side;
  c: Combatant;
  revealed: boolean;
  locked: boolean;
    ...
    <button onClick={() => onSelect(side)} disabled={locked}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ fontSize: 40, fontWeight: 800, fontVariantNumeric: "tabular-nums", lineHeight: 1,
          ...
          <CountUp value={c.pokemon.speed} active={revealed} />
        </div>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ fontSize: 10, color: "var(--color-text-tertiary)", fontVariantNumeric: "tabular-nums" }}>0</span>
        <div style={{ flex: 1 }}>
          <SpeedTierBar value={c.pokemon.speed} revealed={revealed} color={isWinner ? WIN : SPEED_COLOR} compareValue={opponentSpeed} />
        </div>
        <span style={{ fontSize: 10, color: "var(--color-text-tertiary)", fontVariantNumeric: "tabular-nums" }}>{SPEED_MAX}</span>
      </div>
    </button>
  );
}

export default function SpeedGameScreen({ lang }: { lang: Lang }) {
  const t = useTranslations(lang);
  const ts = t.speedGame;
  const { state, actions } = useSpeedGame();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "calc(100dvh - 12rem)" }}>
      <p style={{ textAlign: "center", fontWeight: 700, fontSize: 17, margin: "12px 0 14px", color: "var(--color-text-primary)" }}>{ts.desc}</p>
      ...
      <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", gap: 10, justifyContent: "center",
        ...
        <DuelCard side="a" c={a} revealed={revealed} locked={locked} isWinner={aWin} isWrong={aWrong} opponentSpeed={b.pokemon.speed} lang={lang} onSelect={actions.select} />
        ...
        <DuelCard side="b" c={b} revealed={revealed} locked={locked} isWinner={bWin} isWrong={bWrong} opponentSpeed={a.pokemon.speed} lang={lang} onSelect={actions.select} />
      </div>
